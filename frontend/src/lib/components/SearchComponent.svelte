...
<div class="relative w-full max-w-md">
	...
	<div class="relative">
		...
		<input
			class="input-field pl-10 pr-4 py-3 w-full bg-white border border-neutral-300 rounded-lg focus:ring-2 focus:ring-primary-500 focus:border-transparent transition-all duration-200"
			type="text"
	...
	{#if searchResults && searchResults.length > 0}
		<div class="absolute z-50 w-full mt-2 bg-white rounded-lg shadow-xl border border-neutral-200 max-h-96 overflow-y-auto">
			<div class="py-2">
				{#each searchResults as result}
					...
					>
						<div class="font-medium text-neutral-800 mb-1">{result.title}</div>
						<div class="text-sm text-neutral-600 line-clamp-2">{result.description}</div>
					</a>
				{/each}
			</div>
		</div>
	{:else if searchPattern && searchResults && searchResults.length === 0}
		<div class="absolute z-50 w-full mt-2 bg-white rounded-lg shadow-xl border border-neutral-200">
			<div class="px-4 py-6 text-center">
				...
				<div class="text-neutral-600">No recipes found</div>
				<div class="text-sm text-neutral-500">Try a different search term</div>
			</div>
		</div>
	{/if}
</div>
