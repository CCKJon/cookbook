...
<div class="min-h-screen bg-gradient-to-br from-neutral-50 to-neutral-100 dark:from-neutral-900 dark:to-neutral-800">
	...
	{#if recipe}
			...
			<div class="container-max">
				<div class="grid lg:grid-cols-2 gap-12">
					...
					<div class="bg-white dark:bg-[#0f0805] rounded-xl shadow-lg border border-neutral-100 dark:border-[#080403] p-8">
						<h2 class="text-3xl font-serif font-bold text-neutral-800 dark:text-neutral-100 mb-8 flex items-center gap-3">
						...
						<div class="space-y-4">
							{#each recipe.ingredients as ingredient}
								<div class="flex items-center justify-between p-4 bg-neutral-50 dark:bg-neutral-700 rounded-lg hover:bg-neutral-100 dark:hover:bg-neutral-600 transition-colors duration-200">
									<span class="font-medium text-neutral-800 dark:text-neutral-100">{ingredient.name}</span>
									<span class="text-neutral-600 dark:text-neutral-300">{ingredient.amount}</span>
								</div>
							{/each}
						</div>
					</div>
						...
						<div class="space-y-6">
							{#each recipe.steps as step}
								<div class="flex gap-4">
									<div class="flex-shrink-0 w-8 h-8 bg-primary-600 dark:bg-walnut-600 text-white rounded-full flex items-center justify-center font-bold text-sm">
										{step.step_number}
									</div>
									<div class="flex-1 pt-1">
										<p class="text-neutral-700 dark:text-neutral-200 leading-relaxed">{step.description}</p>
									</div>
								</div>
							{/each}
						</div>
		...
		<!-- Reviews Section -->
		<section class="section-padding bg-white dark:bg-neutral-900 border-t border-neutral-200 dark:border-neutral-700">
			<div class="container-max">
				<h2 class="text-3xl font-serif font-bold text-neutral-800 dark:text-neutral-100 mb-8 text-center">
					Reviews & Ratings
				</h2>

				<!-- Review Form -->
				{#if $authStore.currentUser}
					<div class="bg-white dark:bg-[#0f0805] rounded-xl shadow-lg border border-neutral-100 dark:border-[#080403] p-8 mb-12 max-w-2xl mx-auto">
						<h3 class="text-2xl font-serif font-semibold text-neutral-800 dark:text-neutral-100 mb-6">Share Your Experience</h3>
						
						<form on:submit={submitReview} class="space-y-6">
							<div>
								<label for="title" class="block text-sm font-medium text-neutral-700 dark:text-neutral-300 mb-2">Review Title</label>
								<input
									type="text"
									id="title"
							...
							<div>
								<label for="rating" class="block text-sm font-medium text-neutral-700 dark:text-neutral-300 mb-2">Rating</label>
								<input
									type="number"
									id="rating"
				...
				<div class="space-y-6">
						...
						{#each recipe.review_ids as reviewid}
							<div>
								{#await getReview(reviewid)}
									<div class="bg-white dark:bg-[#0f0805] rounded-xl shadow-lg border border-neutral-100 dark:border-[#080403] p-6 flex items-center justify-center">
										<Spinner color="primary" size="6" />
									</div>
								{:then review}
									<div class="bg-white dark:bg-[#0f0805] rounded-xl shadow-lg border border-neutral-100 dark:border-[#080403] p-6">
										<div class="flex items-start justify-between mb-4">
											<div>
												<h4 class="text-lg font-semibold text-neutral-800 dark:text-neutral-100 mb-1">{review.title}</h4>
												<div class="flex items-center gap-2">
													<div class="flex items-center">
														{#each Array(5) as _, i}
															<svg class="w-4 h-4 {i < review.rating ? 'text-yellow-400' : 'text-neutral-300 dark:text-neutral-600'}" fill="currentColor" viewBox="0 0 20 20">
																<path d="M9.049 2.927c.3-.921 1.603-.921 1.902 0l1.07 3.292a1 1 0 00.95.69h3.462c.969 0 1.371 1.24.588 1.81l-2.8 2.034a1 1 0 00-.364 1.118l1.07 3.292c.3.921-.755 1.688-1.54 1.118l-2.8-2.034a1 1 0 00-1.175 0l-2.8 2.034c-.784.57-1.838-.197-1.539-1.118l1.07-3.292a1 1 0 00-.364-1.118L2.98 8.72c-.783-.57-.38-1.81.588-1.81h3.461a1 1 0 00.951-.69l1.07-3.292z"></path>
															</svg>
														{/each}
													</div>
													<span class="text-sm text-neutral-600 dark:text-neutral-400">{review.rating}/5</span>
												</div>
											</div>
										...
										<p class="text-neutral-700 dark:text-neutral-200 leading-relaxed">{review.review}</p>
									</div>
								{/await}
							</div>
						{/each}
					{:else}
						<div class="text-center py-12">
							...
							<h3 class="text-xl font-semibold text-neutral-800 dark:text-neutral-100 mb-2">No reviews yet</h3>
							<p class="text-neutral-600 dark:text-neutral-300">Be the first to share your experience with this recipe!</p>
						</div>
					{/if}
				</div>
			</div>
